...
| Redis: буфер | да | тело запроса уходит в буфере, а не в сообщении: без него каждый такой запрос получает `error` |
...
| `REDIS_URL` | из `inspector.conf` | буфер: тело и строка запроса по локаторам; проба healthcheck читает только эту переменную |
...
Ожидается `verdict: score` с кодом `VLAI_SCORE`. Без `--redis` проба не кладёт тело в буфер и
...
- **Без `REDIS_URL` контейнер вечно `unhealthy` при живом инспекторе.** Проба кладёт тело в
  буфер и берёт адрес только из окружения; блок `redis` в `inspector.conf` она не читает.
